...
```
[공통] db-architect (db-schema 스킬)
         ↓
[app]  auth-engineer     → backend-engineer      → ui-engineer      → qa-engineer
       (auth-setup)        (facility-backend)       (facility-ui)      (_workspace/app/)

[admin] auth-engineer    → backend-engineer      → ui-engineer      → qa-engineer
        (admin-auth-setup) (admin-backend)          (admin-ui)         (_workspace/admin/)
```
...
**프롬프트 구성:**
```
facility-backend 스킬과 _workspace/01_db_schema.md, _workspace/app/02_auth_session.md를 읽고
apps/app Server Actions와 유틸리티를 구현하라.

타겟 앱: apps/app
- Supabase 클라이언트/유틸은 @spotcare/database에서 import (직접 생성 금지)

산출물:
1. apps/app/app/actions/workspace.ts, facility-type.ts, facility.ts
2. apps/app/lib/validations/*.ts — Zod 검증 스키마
3. _workspace/app/03_backend_api.md — Server Action 시그니처, 반환 타입 목록
...
**프롬프트 구성:**
```
admin-backend 스킬과 _workspace/01_db_schema.md, _workspace/admin/02_auth_session.md를 읽고
apps/admin Server Actions를 구현하라.

타겟 앱: apps/admin
- Supabase 클라이언트는 @spotcare/database에서 import (직접 생성 금지)
- 슈퍼어드민은 tenant_id 필터 없이 전체 데이터 접근

산출물:
1. apps/admin/app/actions/tenant.ts, stats.ts
2. apps/admin/lib/validations/*.ts — Zod 검증 스키마
3. _workspace/admin/03_backend_api.md — Server Action 시그니처, 반환 타입 목록
...
**프롬프트 구성:**
```
facility-ui 스킬과 _workspace/app/03_backend_api.md를 읽고
apps/app 어드민 UI를 구현하라.

타겟 앱: apps/app
- UI 컴포넌트는 @spotcare/ui에서 import (직접 구현 금지)
- 층수 유틸은 @spotcare/database에서 import

산출물:
1. apps/app/app/(auth)/login/page.tsx, signup/page.tsx
2. apps/app/app/dashboard/layout.tsx (사이드바 네비게이션)
3. apps/app/app/dashboard/workspaces/page.tsx (목록 + 생성 Dialog)
...
6. _workspace/app/04_ui_components.md — 주요 컴포넌트 경로 요약
...
**프롬프트 구성:**
```
admin-ui 스킬과 _workspace/admin/03_backend_api.md를 읽고
...
3. apps/admin/app/dashboard/page.tsx (운영 통계)
4. apps/admin/app/dashboard/tenants/page.tsx (테넌트 목록)
5. apps/admin/app/dashboard/tenants/[tenantId]/page.tsx (테넌트 상세)
6. _workspace/admin/04_ui_components.md — 주요 컴포넌트 경로 요약
...
**프롬프트 구성:**
```
_workspace/01_db_schema.md와 _workspace/{target}/ 내 모든 산출물 요약을 읽고
실제 생성된 소스 파일들을 교차 검증하라.

타겟 앱: {apps/app | apps/admin}
공통 검증:
- @spotcare/database, @spotcare/ui import 사용 여부
- {target_app}/ 경로에 파일이 생성되었는지

apps/app 추가 검증:
- 데이터 격리: Server Action의 tenant_id 필터 + RLS 정책 교차 확인
- 층수 변환: floorToDisplay/generateFloorOptions 엣지케이스 (0층, 경계값)
- 인증 흐름: middleware.ts 보호 경로 설정 검증
- 경계면: Server Action 반환 타입 vs UI 컴포넌트 사용 타입 일치 여부

apps/admin 추가 검증:
- 권한 확인: requireAdmin() 모든 Server Action에 적용 여부
- 데이터 노출: tenant_id 필터가 없는지 (슈퍼어드민은 전체 접근이 맞음)
- 인증 흐름: /dashboard 경로 보호 확인
